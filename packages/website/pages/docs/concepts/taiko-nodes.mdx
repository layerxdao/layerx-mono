...
LayerX nodes are minimally modified Ethereum [execution clients](https://ethereum.org/en/glossary/#execution-client) that consist of two parts:
...
The [taiko-geth](https://github.com/taikoxyz/taiko-geth) software is a fork of [go-ethereum](https://github.com/ethereum/go-ethereum) with some changes made according to the LayerX protocol.
...
You can see all the changes made in the the `taiko-geth` fork at [geth.layerx.build](https://geth.layerx.build)!
...
The LayerX protocol allows a block's timestamp to be equal to its parent block's timestamp, which differs from the original Ethereum protocol. So it's fine that there are two `TaikoL1.proposeBlock` transactions included in one L1 block.
</Callout>
  
LayerX client's driver informs the L2 execution engine about LayerX protocol contract's latest verified L2 head and tries to let it catch up with the latest verified L2 block through P2P at first.
...
3. Checks whether the `txList` is valid based on the rules defined in the LayerX protocol.
...
<Callout type="info" emoji="ℹ️">
Read more about block metadata [here](https://layerx.build/docs/concepts/creating-taiko-blocks/intrinsic-validity-functions) or see `5.2.2 Block Metadata` in the [whitepaper](https://taikoxyz.github.io/layerx-mono/taiko-whitepaper.pdf).

Read more about `txList` validation rules [here](https://layerx.build/docs/concepts/creating-taiko-blocks/intrinsic-validity-functions) or see `5.3.1 Validation` in the [whitepaper](https://taikoxyz.github.io/layerx-mono/taiko-whitepaper.pdf).

Read more about the `TaikoL2.anchor` transaction and proposed block's determination [here](https://layerx.build/docs/concepts/creating-taiko-blocks/anchor-transaction) or see `5.4.1 Construction of Anchor Transactions` in the [whitepaper](https://taikoxyz.github.io/layerx-mono/taiko-whitepaper.pdf).
</Callout>
...
2. If there are too many pending transactions in the L2 execution engine, splits them into several smaller `txLists`. This is because the LayerX protocol restricts the max size of each proposed `txList`.
...
<Callout type="info" emoji="ℹ️">
Read more about why we need these Merkle proofs when proving in `5.5 Proving Blocks` in the [whitepaper](https://taikoxyz.github.io/layerx-mono/taiko-whitepaper.pdf).
</Callout>
