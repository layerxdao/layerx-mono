...
This guide will help you interact with Swap, which is a fork of Uniswap v2 that LayerX has deployed only for testing purposes.

## Prerequisites

- You must have some tokens (at least ETH) on LayerX (Alpha-3 testnet). You can [receive some ETH, HORSE, and BLL from the faucets](/docs/guides/receive-tokens) and then [bridge them to LayerX](/docs/guides/use-the-bridge).

## Steps

<Steps>
    ### Navigate to Swap
    Navigate to [https://swap.test.layerx.build](https://swap.test.layerx.build).

    ### Connect to the LayerX testnet
