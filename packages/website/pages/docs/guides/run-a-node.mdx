...
This guide will walk you through the process of operating a LayerX node via [simple-taiko-node](https://github.com/taikoxyz/simple-taiko-node). You will be able to:

- Run a LayerX node easily from the command line on Windows, Mac, and Linux.
...
- Consult the [Geth minimum hardware requirements](https://github.com/ethereum/go-ethereum#hardware-requirements), but ignore the storage requirement as LayerX nodes will require less storage. Around 50GB should be more than enough initially, but over time it could become insufficient as the chain grows. As of `2023-06-21T16:59:11+00:00` a LayerX node sync takes **17.3 GB**.
...
<Tabs items={["Local", "Alchemy", "Infura"]} defaultIndex="0">
  ...
  - for "name", enter "LayerX A3 Node", or your preferred name. 
...
<Callout>
  If you ran a testnet node previously make sure to first run a `docker compose down -v` to remove the old volumes. Also, it may take some time to synchronize from the genesis block. You can monitor this progress through logs, or in the local grafana dashboard and see the latest L2 chain status in the [LayerX block explorer](https://explorer.test.layerx.build/).
</Callout>
